...
function checkCashRegister(price, cash, cid) {
    ...
    let lastDrawerRegister = 0;
    for(let x of cid) {
      lastDrawerRegister = lastDrawerRegister + x[1];
    } //totalizador
            
    lastDrawerRegister = lastDrawerRegister.toFixed(2);  
            
    let change = cash - price;
    const changeArray = [];
    
    if (change > lastDrawerRegister) {
         return { status: "INSUFFICIENT_FUNDS", change: changeArray };
        } 
        else if (change.toFixed(2) === lastDrawerRegister) {
