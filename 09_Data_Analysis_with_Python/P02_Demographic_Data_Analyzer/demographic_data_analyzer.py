...
def calculate_demographic_data(print_data=True):
  # Read data from file
  df = pd.read_csv('adult.data.csv')  #OK

  # How many of each race are represented in this dataset?
  # This should be a Pandas series with race names as the index labels.
  race_count = df['race'].value_counts()  #OK
  ...
  # What percentage of people with and without `Bachelors`, `Masters`, or `Doctorate` make more than 50K?

  # What is the minimum number of hours a person works per week (hours-per-week feature)?
  min_work_hours = df['hours-per-week'].min()  #OK
  ...
  highest_earning_country_percentage = round(
      (df[(df['native-country'] == highest_earning_country) &
       ...
       df[df['native-country'] == highest_earning_country].shape[0]) * 100, 1)

  # Identify the most popular occupation for those who earn >50K in India.
  top_IN_occupation = df[(df['salary'] == '>50K') & (
      df['native-country'] == 'India')]['occupation'].value_counts().idxmax()
